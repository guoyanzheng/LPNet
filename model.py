...
from hdc_duc import HDC, DUC
import torch.nn.functional as F


# 3d unet model borrow from : https://github.com/wolny/pytorch-3dunet
...
def create_conv(in_channels,
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            assert num_channels % num_groups == 0, 'Expected number of channels in input to be divisible by num_groups. num_channels={}, num_groups={}'.format(
            ...
            raise ValueError(
                "Unsupported layer type '{}'. MUST be one of ['b', 'g', 'r', 'l', 'e', 'c']"
                    .format(char))
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_features, x):
        if self.upsample is None:
            ...
            x = F.interpolate(x, size=output_size, mode='nearest')  # interpolation in decoder
...
class UNet3D_HDC(nn.Module):
    ...
    Args:
        ...
        num_groups (int): number of groups for the GroupNorm
        number_of_fmas (int): depth of the model
        scale_factor (int or tuple or list): Scale up and scale down factors.
    Notes:
        input data shape : (batchsize,channels,height,width,depth) or (batch,channels,x,y,z)
    """
    ...
    def __init__(self,
                 ...
                 scale_factor=2,
                 **kwargs):
        super(UNet3D_HDC, self).__init__()

        self.in_hdc = HDC(scale_factor)

        if isinstance(f_maps, int):
            # use number_of_fmaps levels in the encoder path as suggested in the paper
            f_maps = create_feature_maps(f_maps,
                                         number_of_fmaps=number_of_fmaps)
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                encoder = Encoder(in_channels,
                                  out_feature_num,
                                  apply_pooling=False,  # no pooling in the first layer
                                  basic_module=DoubleConv,
        ...
        self.out_duc = DUC(scale_factor, out_channels, f_maps[0])
        if final_sigmoid:
    ...
    def forward(self, x):
        ...
        for encoder in self.encoders:
            ...
            encoders_features.insert(
                0, x)  # insert(index,object)
        ...
        if not self.training:
            x = self.final_activation(x)  # training also output sigmoid value
